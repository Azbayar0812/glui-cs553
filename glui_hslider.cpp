...
// MJB
// fprintf( stderr, "set_float_limits(): float_low = %f, float_high = %f\n", float_low, float_high );
// fprintf( stderr, "set_float_limits(): button_left_edge = %f, button_right_edge = %f\n", button_left_edge, button_right_edge );
...
// MJB
// fprintf( stderr, "update_slider_val(): ptr_val = 0x%08x\n", (int)ptr_val );
// fprintf( stderr, "update_slider_val(): live_type = %d\n", live_type );
// fprintf( stderr, "update_slider_val(): GLUI_LIVE_FLOAT = %d\n", GLUI_LIVE_FLOAT );
   fp = (float*) ptr_val;
   ip = (int*)   ptr_val;
   ...
   if( button_is_resizable )
   {
      ...
      if( live_type == GLUI_LIVE_FLOAT )
         fp[1] = button_right_edge;
      else
         ip[1] = round( button_right_edge );
   }

   if( live_type == GLUI_LIVE_FLOAT )
      fp[0] = button_left_edge;
   else
      ip[0] = round( button_left_edge );

// MJB
// fprintf( stderr, "update_slider_val(): left = %f, right = %f\n", left, right );
// fprintf( stderr, "update_slider_val(): button_left_edge = %f, button_right_edge = %f\n", button_left_edge, button_right_edge );
// fprintf( stderr, "update_slider_val(): fp[0] = %f, fp[1] = %f\n", fp[0], fp[1] );
